...
class ImportDateController(QDialog):
    ...
    def send_date_signal(self):        
        ...
        self.start_date = self.ui.dateinput_button.date().toPython().strftime("%d-%b-%Y")
        self.ui.progress_text.clear()
        self.ui.progress_text.append(f"IMPORTANDO DOCUMENTOS DESDE: {self.start_date}\n")
    ...
    def run(self):
        ...
        if not self.started:
            self.started = True
            self.run_threaded_process(self.import_documents, self.progress_function, self.finished_function)
    ...
    def import_documents(self, progress_callback):
        ...
        if mail_downloader.connection:
            mail_downloader.get_mail_data(self.start_date)
...
class Worker(QRunnable):
    ...
    def run(self):
        try:
            ...
            self.signals.error.emit((excemption_type, value, traceback.format_exc()))
