...
class XmlDocument:
    ...
    detalle_mensaje="", xml_documento="", xml_respuesta=""):
    ...
    def verify_registered_edoc(self):
        ...
        if connection:
            cursor = connection.cursor()
            cursor.execute(f"""SELECT id FROM documento_electronico WHERE clave = ?""", (self.clave, ))
    ...
    def save_edoc_data(self):
        ...
        if connection:
            ...
            self.xml_documento, self.xml_respuesta))
            connection.commit()
            return True
        else:
            print("error")
    ...
    def update_edoc_response(self):
        ...
        if connection:
            ...
            cursor.execute(query_str, (self.respuesta_hacienda, self.detalle_mensaje, self.xml_respuesta, self.clave, ))
            connection.commit()
            return True
    ...
    def insert_edoc_response(self):
        ...
        if connection:
            ...
            cursor.execute(query_str, (self.clave, self.respuesta_hacienda, self.detalle_mensaje, self.xml_respuesta, ))
            connection.commit()
            return True
    ...
    def set_document_type(self):
        ...
        if connection:
            cursor = connection.cursor()
            cursor.execute(f"""SELECT id FROM documento_tipo WHERE codigo = ?""", (self.documento_tipo, ))
            data = cursor.fetchone()
    ...
    def update_edoc_data(self):
        ...
        if connection:
            ...
            exento = ?, exonerado = ?, subtotal = ?, descuento = ?, impuesto = ?, otros = ?, total = ?, xml_documento  = ? WHERE clave = ?"""
            cursor.execute(query_str, 
            (self.proveedor, self.cedula, self.documento_tipo, self.consecutivo, self.fecha, self.moneda, self.gravado, self.exento,
            self.exonerado, self.subtotal, self.descuento, self.impuesto, self.otros, self.total, self.xml_documento, self.clave))
            connection.commit()
            return True
        else:
            print("error")
    ...
    def get_provider_list(self):
        ...
        if connection:
            cursor = connection.cursor()
            cursor.execute(f"""SELECT DISTINCT proveedor FROM documento_electronico ORDER BY proveedor""")
            data = cursor.fetchall()
            return data
        else:
            return []
