...
class Caronte(QMainWindow):
    ...
    def search_edocs(self):
        ...
        self.start_date, self.end_date = self.get_dates(self.ui.fecha_desde.date(), self.ui.fecha_hasta.date())
        ...
        self.document_list = edocs_data.get_edoc_data(query_string, tuple(query_list))
    ...
    def add_to_table_widget(self, document_list):
        ...
        for row_number, row in enumerate(self.document_list):
            for column_number, column in enumerate(row):
                if 7 <= column_number <= 14:
                    self.ui.tableWidget.setItem(row_number, column_number, QTableWidgetItem("{0:,.2f}".format(float(column))))
                else:
                    self.ui.tableWidget.setItem(row_number, column_number, QTableWidgetItem(column))

        self.ui.tableWidget.resizeColumnsToContents()

    def export_data(self):
        if len(self.document_list) > 0:
                file_path = QFileDialog.getSaveFileName(self, "Guardar Archivo Excel", "/", filter="xlsx (*.xlsx)")
                ...
                sheet1.write_string(4, 0, f"Documentos Recibidos desde: {self.start_date} hasta: {self.end_date}")
                ...
                for row_number, row in enumerate(self.document_list):
                    for column_number, column in enumerate(row):
                        if column is None:
                            column = ""
                        if 7 <= column_number <= 14:
                            sheet1.write_number(row_number + row_counter, column_number, column, currency_format)
                        else:
                            sheet1.write_string(row_number + row_counter, column_number, column, text_format)

                try:
                    book.close()
                    QMessageBox.information(self, "Exito", "Archivo guardado en: {}".format(file_path[0]))
                except:
    ...
    def setup_table_headers(self):
        self.header_list = ["Proveedor", "Cedula", "Tipo Documento", "Clave", "Consecutivo", "Fecha", 
        "Moneda", "Gravado", "Exento", "Exonerado", "SubTotal", "Descuento", "Impuesto", "Otros", "Total", "Resp. Hacienda", "Detalle"]
